					...
					if ( zakra_is_woocommerce_active() && function_exists( 'is_woocommerce' ) && is_woocommerce() ) {
						...
						if ( 'page-header' === get_theme_mod( 'zakra_page_title_enabled', 'page-header' ) ) {
							...
							woocommerce_breadcrumb(
								array(
									'wrap_before' => '<nav role="navigation" aria-label="' . esc_html__( 'Breadcrumbs', 'zakra' ) . '" class="breadcrumb-trail breadcrumbs" itemprop="breadcrumb""><ul class="trail-items" itemscope="" itemtype="http://schema.org/BreadcrumbList">',
									'wrap_after'  => '</ul></nav>',
									'before'      => '<li itemprop="itemListElement" itemscope="" itemtype="http://schema.org/ListItem" class="trail-item">',
									'after'       => '</li>',
									'delimiter'   => '',
								)
							);
